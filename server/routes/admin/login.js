    ...
    if (!String.isEmpty(manager.id)) {
        ...
        db.query(`SELECT * FROM managers WHERE manager_id LIKE ?`, [manager.id], (err, managerDB) => {
            ...
            if (String.isEmpty(managerDB)) {
                ...
                // 비밀번호 해쉬 값 찾기
                crypto.pbkdf2(manager.passwd, managerDB[0].manager_salt, 100000, 64, 'sha512', (err, key) => {
                    passwd = key.toString('hex');

                    if (managerDB != false && managerDB[0].manager_id == manager.id && managerDB[0].manager_passwd == passwd) {
    ...
    if (!String.isEmpty(manager.id)) {
        ...
        crypto.randomBytes(64, (err, buf) => {
            ...
            crypto.pbkdf2(manager.passwd, salt, 100000, 64, 'sha512', (err, key) => {
                ...
                // DB 등록
                db.query(`INSERT INTO managers(manager_id, manager_name, manager_email, manager_passwd, manager_salt) VALUES(?, ?, ?, ?, ?)`, [manager.id, manager.name, manager.email, passwd, salt], (err, result) => {
                    if (err) {
                        // 실패시 false 응답
                        response.success = false;
                        response.msg = err;
